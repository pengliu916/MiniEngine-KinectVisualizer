...
FXAA::CreateResource()
{
    ...
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &Pass1ComputeShader));
    macro[1].Definition = "0"; // define Pass1 0
    macro[6].Definition = "1"; // define Color2Luma 1
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &Color2LumaComputeShader));
    macro[6].Definition = "0"; // define Pass1 0
    macro[2].Definition = "1"; // define ResolveWork 1
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &ResolveWorkComputeShader));
    macro[2].Definition = "0"; // define ResolveWork 0
    macro[3].Definition = "1"; // define Pass2 1
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &Pass2HComputeShader));
    macro[4].Definition = "1"; // define Vertical_Orientation 1
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &Pass2VComputeShader));
    macro[5].Definition = "1"; // define Debug_Output 1
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &Pass2VDebugComputeShader));
    macro[4].Definition = "0"; // define Vertical_Orientation 0
    V(Graphics::CompileShaderFromFile(Core::GetAssetFullPath(
        _T("FXAA.hlsl")).c_str(), macro, D3D_COMPILE_STANDARD_FILE_INCLUDE,
        "csmain", "cs_5_1", compileFlags, 0, &Pass2HDebugComputeShader));
...
#undef  CreatePSO
    ...
    IndirectParameters.Create(L"FXAA Indirect Parameters",
        2, sizeof(D3D12_DISPATCH_ARGUMENTS), initArgs);
...
FXAA::Resize()
{
    ...
    g_LumaBuffer.Create(L"Luma Buffer",
        Graphics::g_SceneColorBuffer.GetWidth(),
        Graphics::g_SceneColorBuffer.GetHeight(), 1, DXGI_FORMAT_R8_UNORM);
}
...
void
FXAA::Render(ComputeContext& Context)
{
    GPU_PROFILE(Context, L"FXAA");
    Context.SetRootSignature(RootSig);
    Context.SetConstants(0, 1.f / Graphics::g_SceneColorBuffer.GetWidth(),
        1.f / Graphics::g_SceneColorBuffer.GetHeight(),
        ContrastThreeshold, SubpixelRemoval);

    // Pass1
    Context.ResetCounter(g_FXAAWorkQueueH);
    Context.ResetCounter(g_FXAAWorkQueueV);

    Context.TransitionResource(Graphics::g_SceneColorBuffer,
        D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
    Context.TransitionResource(g_FXAAWorkQueueH,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    Context.TransitionResource(g_FXAAWorkQueueV,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    Context.TransitionResource(g_FXAAColorQueueH,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    Context.TransitionResource(g_FXAAColorQueueV,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    Context.TransitionResource(g_LumaBuffer,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    ...
    Context.Dispatch2D(Graphics::g_SceneColorBuffer.GetWidth(),
        Graphics::g_SceneColorBuffer.GetHeight());
    // Pass2
    Context.SetPipelineState(ResolveWorkCS);
    Context.TransitionResource(IndirectParameters,
        D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
    ...
    Context.SetDynamicDescriptors(2, 0, 1,
        &g_FXAAWorkQueueH.GetCounterSRV(Context));
    Context.SetDynamicDescriptors(2, 1, 1,
        &g_FXAAWorkQueueV.GetCounterSRV(Context));
    ...
    Context.SetDynamicDescriptors(1, 0, 1,
        &Graphics::g_SceneColorBuffer.GetUAV());
    ...
    // The final phase involves processing pixels on the work queues and
    // writing them back into the color buffer. Because the two source pixels
    // required for linearly blending are held in the work queue, this does not
    // require also sampling from the target color buffer
    // (i.e. no read/modify/write, just write.)
...
FXAA::UpdateGUI()
{
    if (ImGui::CollapsingHeader("FXAA")) {
        ...
        ImGui::SliderFloat("Contrast Threshold",
            &ContrastThreeshold, 0.05f, 0.5f);
        ImGui::SliderFloat("Subpixel Removal",
            &SubpixelRemoval, 0.f, 1.f);
