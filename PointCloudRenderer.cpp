...
HRESULT PointCloudRenderer::OnCreateResource()
{
	...
	m_RootSignature[1].InitAsDescriptorRange( D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 0, 2 );
	...
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"PointCloudRenderer.hlsl" ).c_str(), 
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vs_heightmap_main", "vs_5_1", compileFlags, 0, &HightMapVS ) );
	macro[1].Definition = "0";
	macro[2].Definition = "1";
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"PointCloudRenderer.hlsl" ).c_str(),
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vs_passthrough_main", "vs_5_1", compileFlags, 0, &PassThroughVS ) );
	macro[2].Definition = "0";
	macro[3].Definition = "1";
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"PointCloudRenderer.hlsl" ).c_str(), 
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_tex_main", "ps_5_1", compileFlags, 0, &TexPS ) );
	macro[3].Definition = "0";
	macro[4].Definition = "1";
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"PointCloudRenderer.hlsl" ).c_str(), 
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "gs_normalsurface_main", "gs_5_1", compileFlags, 0, &NormalSurfaceGS ) );
	macro[4].Definition = "0";
	macro[5].Definition = "1";
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"PointCloudRenderer.hlsl" ).c_str(), 
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "gs_surface_main", "gs_5_1", compileFlags, 0, &SurfaceGS ) );
	macro[5].Definition = "0";
	macro[6].Definition = "1";
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"PointCloudRenderer.hlsl" ).c_str(),
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_texnormal_main", "ps_5_1", compileFlags, 0, &TexNormalPS) );

	m_RenderModePSO[kPointCloud].SetRootSignature( m_RootSignature );
	m_RenderModePSO[kPointCloud].SetRasterizerState( Graphics::g_RasterizerTwoSided );
	m_RenderModePSO[kPointCloud].SetBlendState( Graphics::g_BlendDisable );
	m_RenderModePSO[kPointCloud].SetDepthStencilState( Graphics::g_DepthStateReadWrite );
	m_RenderModePSO[kPointCloud].SetSampleMask( 0xFFFFFFFF );
	m_RenderModePSO[kPointCloud].SetInputLayout( 0, nullptr );
	m_RenderModePSO[kPointCloud].SetPixelShader( TexPS->GetBufferPointer(), TexPS->GetBufferSize() );
	m_RenderModePSO[kPointCloud].SetRenderTargetFormats( 1, &Graphics::g_SceneColorBuffer.GetFormat(), Graphics::g_SceneDepthBuffer.GetFormat() );
	m_RenderModePSO[kSurface] = m_RenderModePSO[kPointCloud];
	m_RenderModePSO[kSurface].SetPrimitiveTopologyType( D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT );
	m_RenderModePSO[kSurface].SetVertexShader( PassThroughVS->GetBufferPointer(), PassThroughVS->GetBufferSize() );
	m_RenderModePSO[kShadedSurface] = m_RenderModePSO[kSurface];
	m_RenderModePSO[kShadedSurface].SetGeometryShader( NormalSurfaceGS->GetBufferPointer(), NormalSurfaceGS->GetBufferSize() );
	m_RenderModePSO[kShadedSurface].SetPixelShader( TexNormalPS->GetBufferPointer(), TexNormalPS->GetBufferSize() );
	m_RenderModePSO[kShadedSurface].Finalize();
	m_RenderModePSO[kSurface].SetGeometryShader( SurfaceGS->GetBufferPointer(), SurfaceGS->GetBufferSize() );
	m_RenderModePSO[kSurface].Finalize();
	m_RenderModePSO[kPointCloud].SetVertexShader( HightMapVS->GetBufferPointer(), HightMapVS->GetBufferSize() );
	m_RenderModePSO[kPointCloud].SetPrimitiveTopologyType( D3D12_PRIMITIVE_TOPOLOGY_TYPE_POINT );
	m_RenderModePSO[kPointCloud].Finalize();

	return hr;
}

void PointCloudRenderer::UpdateCB( DirectX::XMFLOAT2 ColReso, DirectX::XMFLOAT2 DepReso, 
...
void PointCloudRenderer::OnRender( GraphicsContext& gfxContext, ColorBuffer* pDepthMap, ColorBuffer* pColorMap, XMMATRIX viewProj )
{
	...
	gfxContext.SetDynamicConstantBufferView( 0, sizeof( CBuffer ), (void*)&m_CBData );
