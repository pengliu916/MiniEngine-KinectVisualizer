...
RootSignature::RootSignature(
    UINT NumRootParams /* = 0 */, UINT NumStaticSamplers /* = 0 */)
...
void
RootSignature::Reset(UINT NumRootParams, UINT NumStaticSamplers /* = 0 */)
{
    if (NumRootParams > 0) {
...
const RootParameter&
RootSignature::operator [](size_t EntryIndex) const
{
    ASSERT(EntryIndex < m_NumParameters);
    return m_ParamArray.get()[EntryIndex];
}
...
void 
RootSignature::Finalize(const std::wstring& Name,
    D3D12_ROOT_SIGNATURE_FLAGS Flags /* = D3D12_ROOT_SIGNATURE_FLAG_NONE */)
{
    ...
    if (firstCompile) {
        ...
        V(Graphics::g_device->CreateRootSignature(
            1, pOutBlob->GetBufferPointer(),
