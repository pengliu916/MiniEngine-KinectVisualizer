...
#if HeightMapVS
// [NOTE] when use 'out' keyword: the order of next stage input should match previous stage out order
void vs_heightmap_main( in uint VertID : SV_VertexID, out float2 Tex : TEXCOORD0, out float4 Pos : SV_Position )
{
...
float3 Shade( float4 Pos, float3 Nor )
{
	...
	light_dir.w = clamp( 1.0f / (LightAttn.x + LightAttn.y * light_dist + LightAttn.z * light_dist * light_dist), 0, 1 );
...
// Use gs for this is not optimal. Best solution should be using indexed heighmap mesh in VS for vertex reuse
[maxvertexcount( 6 )]
void gs_normalsurface_main( point uint VertID[1] : POSITION0, inout TriangleStream<TexColPos> TriStream )
{
	TexColPos output;
	uint2 xy = uint2(VertID[0] % DepthInfraredReso.x, VertID[0] / DepthInfraredReso.x);
	uint z[4] = {DepthMap[xy + offset[0]], DepthMap[xy + offset[1]], DepthMap[xy + offset[2]], DepthMap[xy + offset[3]]};
	uint2 minmax = z[0] > z[1] ? uint2(z[1], z[0]) : uint2(z[0], z[1]);
	minmax.x = min( min( minmax.x, z[2] ), z[3] );
	minmax.y = max( max( minmax.y, z[2] ), z[3] );

	if ((minmax.y - minmax.x)*0.001f > MaxQuadDepDiff) return;
	...
	[unroll] for (int i = 0; i < 4; i++)
	{
		...
		Pos[i] = float4((_xy - DepthCxyFxy.xy) / DepthCxyFxy.zw * depth, depth, 1.f);
...
void gs_surface_main( point uint VertID[1] : POSITION0, inout TriangleStream<TexPos> TriStream )
{
	...
	[unroll] for (int i = 0; i < 4; i++)
	{
		uint2 _xy = xy + offset[i];
		float depth = z[i] * 0.001f;
		float4 pos = float4((_xy - DepthCxyFxy.xy) / DepthCxyFxy.zw * depth, depth, 1.f);
		float4 pos_col = mul( Depth2Color, pos );
		pos = pos * float4(1.f, -1.f, 1.f, 1.f) + Offset;
...
#if TexNormalPS
float4 ps_texnormal_main( float2 Tex : TEXCOORD0, float3 Col : COLOR0) : SV_Target0
{
