...
HRESULT KinectVisualizer::OnCreateResource()
{
    ...
    m_BilateralFilter.UpdateCB(
        XMUINT2(DepWidth, DepHeight), DXGI_FORMAT_R16_UINT);
    OnSizeChanged();
...
void KinectVisualizer::OnRender(CommandContext & EngineContext)
{
    ...
    if (_BilateralFilter) {
        m_BilateralFilter.OnRender(EngineContext.GetGraphicsContext(), 
            &m_SensorTexGen.m_OutTexture[SensorTexGen::kDepthTex]);
    }
    ...
    m_PointCloudRenderer.OnRender(EngineContext.GetGraphicsContext(),
        &m_SensorTexGen.m_OutTexture[SensorTexGen::kDepthTex],
    ...
    switch (msg->message) {
        ...
        case WM_POINTERUP: {
            ...
            if (msg->message == WM_POINTERUP) {
                m_Camera.RemovePointer(pointerId);
            }
        }
    }
    return false;
