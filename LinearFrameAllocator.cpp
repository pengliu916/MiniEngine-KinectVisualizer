...
// KinectBuffer
//--------------------------------------------------------------------------------------
KinectBuffer::KinectBuffer( DXGI_FORMAT Format, uint32_t NumElements, uint32_t ElementSize )
	:m_ElementCount( NumElements ), m_ElementSize( ElementSize ), m_DataFormat( Format )
{
...
//--------------------------------------------------------------------------------------
LinearFrameAllocator::LinearFrameAllocator( uint32_t NumElements, uint32_t ElementSize, DXGI_FORMAT Format )
	: m_NumElements( NumElements ), m_ElementSize( ElementSize ), m_BufferFormat( Format )
{
}
...
{
	while (!m_RetiredBuffers.empty() && Graphics::g_cmdListMngr.IsFenceComplete( m_RetiredBuffers.front().first ))
	{
...
void LinearFrameAllocator::DiscardBuffer( uint64_t FenceID, KinectBuffer* Buffer )
{
...
{
	KinectBuffer* BufferPtr = new KinectBuffer( m_BufferFormat,m_NumElements,m_ElementSize );
	return BufferPtr;
}