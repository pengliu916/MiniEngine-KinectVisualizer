...
HRESULT SeperableFilter::OnCreateResoure( DXGI_FORMAT BufferFormat)
{
	...
	V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"BilateralFilter.hlsl" ).c_str(),
		macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "vs_quad_main", "vs_5_1", compileFlags, 0, &QuadVS) );
	...
	for (int i = 0; i < kNumKernelDiameter; ++i)
	{
		...
		V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"BilateralFilter.hlsl" ).c_str(),
			macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_bilateralfilter_main", "ps_5_1", compileFlags, 0, &VerticalPS[i] ) );
		macro[3].Definition = "1";
		V( Graphics::CompileShaderFromFile( Core::GetAssetFullPath( L"BilateralFilter.hlsl" ).c_str(),
			macro, D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_bilateralfilter_main", "ps_5_1", compileFlags, 0, &HorizontalPS[i] ) );
		...
		m_HPassPSO[i].SetVertexShader( QuadVS->GetBufferPointer(), QuadVS->GetBufferSize() );
		m_HPassPSO[i].SetRenderTargetFormats( 1, &BufferFormat, DXGI_FORMAT_UNKNOWN );
		m_HPassPSO[i].SetPrimitiveTopologyType( D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE );
		m_VPassPSO[i] = m_HPassPSO[i];
		m_VPassPSO[i].SetPixelShader( VerticalPS[i]->GetBufferPointer(), VerticalPS[i]->GetBufferSize() );
		m_VPassPSO[i].Finalize();
		m_HPassPSO[i].SetPixelShader( HorizontalPS[i]->GetBufferPointer(), HorizontalPS[i]->GetBufferSize() );
...
void SeperableFilter::UpdateCB( DirectX::XMUINT2 Reso, DXGI_FORMAT Format )
{
	if (m_CBData.Reso.x != Reso.x || m_CBData.Reso.y != Reso.y)
	{
		...
		m_WorkingBuffer.Create( L"BilateralTemp", (uint32_t)Reso.x, (uint32_t)Reso.y, 1, Format );
		m_CBData.Reso = Reso;
...
void SeperableFilter::OnRender( GraphicsContext& gfxContext, ColorBuffer* pInputTex )
{
	...
	gfxContext.SetRenderTargets( 1, &m_WorkingBuffer );
	gfxContext.SetViewport( m_Viewport );
	gfxContext.SetScisor( m_ScisorRact );
	gfxContext.TransitionResource( m_WorkingBuffer, D3D12_RESOURCE_STATE_RENDER_TARGET );
	gfxContext.Draw( 3 );

	gfxContext.TransitionResource( m_WorkingBuffer, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
	gfxContext.SetPipelineState( m_VPassPSO[m_KernelSizeInUse] );
	gfxContext.SetDynamicDescriptors( 1, 0, 1, &m_WorkingBuffer.GetSRV() );
	gfxContext.SetRenderTargets( 1, pInputTex );
	gfxContext.Draw( 3 );
}
...
void SeperableFilter::RenderGui()
{
	...
	{
		ImGui::SliderInt( "Kernel Radius", (int*)&m_KernelSizeInUse, 0, kNumKernelDiameter - 1 );
		ImGui::DragFloat( "Range Variance", &m_CBData.DiffrenceVariance, 1.f, 100, 2500 );
	}
}