...
#include "TextRenderer_SharedHeader.inl"
    ...
    public:
        ...
        // Each character has an XY start offset, a width,
        // and they all share the same height
        struct Glyph {
            uint16_t x, y, w;
            int16_t bearing;
            uint16_t advance;
...
{
public:
    TextContext(GraphicsContext& CmdContext,
        float canvasWidth = 1920.f, float canvasHeight = 1080.f);
    ...
    //====================================================================
    // Choose a font from the Fonts folder.
    // Previously loaded fonts are cached in memory.
    void SetFont(const std::wstring& fontName, float TextSize = 0.0f);

    // Resize the view space. This determines the coordinate space of the
    // cursor position and font size. You can always set the view size to
    // the same dimensions regardless of actual display resolution. It is
    // assumed, however, that the aspect ratio of this virtual coordinate
    // system matches the actual display aspect ratio.
    void SetViewSize(float ViewWidth, float ViewHeight);

    // Set the size of the text relative to the ViewHeight. The aspect of
    // the text is preserved from the TTF as long as the aspect ratio of
    // the view space is the same as the actual viewport.
    ...
    // A more powerful function which formats text like printf(). Very
    // slow by comparison, so use it only if you're going to format text anyway.
...
private:
    ...
    __declspec(align(16)) struct TextVert {
        // Upper-left glyph position in screen space
        float X, Y;
        // Upper-left glyph UV and the width in texture space
        uint16_t U, V, W, H;
    };

    UINT FillVertexBuffer(
        TextVert volatile* verts, const char* str, size_t stride, size_t slen);
    ...
    // Percentage of the font's TextSize should the shadow be offset
