...
{
    m_pKinect2 = StreamFactory::createFromKinect2(
...
void SensorTexGen::OnCreateResource()
{
    ...
    m_pFrameAlloc[IRGBDStreamer::kDepth] = new LinearFrameAllocator(
        DepWidth * DepHeight, sizeof(uint16_t), DXGI_FORMAT_R16_UINT);
    m_pKinectBuffer[IRGBDStreamer::kDepth] = 
        m_pFrameAlloc[IRGBDStreamer::kDepth]->RequestFrameBuffer();
    m_OutTexture[kDepthVisualTex].Create(L"Depth Visual Tex", 
        DepWidth, DepHeight, 1, DXGI_FORMAT_R11G11B10_FLOAT);
    m_OutTexture[kDepthTex].Create(L"Depth Raw Tex", 
        DepWidth, DepHeight, 1, DXGI_FORMAT_R16_UINT);

    m_pFrameAlloc[IRGBDStreamer::kInfrared] = new LinearFrameAllocator(
        DepWidth * DepHeight, sizeof(uint16_t), DXGI_FORMAT_R16_UINT);
    m_pKinectBuffer[IRGBDStreamer::kInfrared] = 
        m_pFrameAlloc[IRGBDStreamer::kInfrared]->RequestFrameBuffer();
    m_OutTexture[kInfraredTex].Create(L"Infrared Tex", 
        DepWidth, DepHeight, 1, DXGI_FORMAT_R11G11B10_FLOAT);
    ...
    m_pFrameAlloc[IRGBDStreamer::kColor] = new LinearFrameAllocator(
        ColWidth * ColHeight, sizeof(uint32_t), DXGI_FORMAT_R8G8B8A8_UINT);
    m_pKinectBuffer[IRGBDStreamer::kColor] = 
        m_pFrameAlloc[IRGBDStreamer::kColor]->RequestFrameBuffer();
    m_OutTexture[kColorTex].Create(L"Color Tex", 
        ColWidth, ColHeight, 1, DXGI_FORMAT_R11G11B10_FLOAT);

    for (uint16_t i = 0; i < kNumTargetTex; ++i) {
        m_OutTextureRTV[i] = m_OutTexture[i].GetRTV();
    }
    ...
    m_RootSignature[2].InitAsDescriptorRange(
        D3D12_DESCRIPTOR_RANGE_TYPE_UAV, 0, IRGBDStreamer::kNumBufferTypes + 1);
    m_RootSignature.Finalize(L"SensorTexGen",
    ...
    V(Graphics::CompileShaderFromFile(
        Core::GetAssetFullPath(_T("SensorTexGen.hlsl")).c_str(), macro, 
    ...
    for (int i = 0; i < kNumDataMode; ++i) {
        ...
        for (int j = 0; j < kNumDepthMode; ++j) {
            ...
            V(Graphics::CompileShaderFromFile(
                ...
                "ps_copy_depth_infrared_main", "ps_5_1", 
            ...
            V(Graphics::CompileShaderFromFile(
                Core::GetAssetFullPath(_T("SensorTexGen.hlsl")).c_str(), macro,
                D3D_COMPILE_STANDARD_FILE_INCLUDE, 
                "cs_copy_depth_infrared_main", "cs_5_1", 
        ...
        V(Graphics::CompileShaderFromFile(
            Core::GetAssetFullPath(_T("SensorTexGen.hlsl")).c_str(), macro,
            D3D_COMPILE_STANDARD_FILE_INCLUDE, "ps_copy_color_main", "ps_5_1", 
            compileFlags, 0, &CopyColorPS[0][i]));
        macro[2].Definition = "0"; // CopyPS
        macro[3].Definition = "1"; // CopyCS
        V(Graphics::CompileShaderFromFile(
            Core::GetAssetFullPath(_T("SensorTexGen.hlsl")).c_str(), macro,
            D3D_COMPILE_STANDARD_FILE_INCLUDE, "cs_copy_color_main", "cs_5_1", 
    ...
    for (int i = 0; i < kNumDataMode; ++i) {
        ...
        m_GfxColorPSO[0][i].SetDepthStencilState(
            Graphics::g_DepthStateDisabled);
        m_GfxColorPSO[0][i].SetSampleMask(0xFFFFFFFF);
        m_GfxColorPSO[0][i].SetInputLayout(0, nullptr);
        m_GfxColorPSO[0][i].SetPrimitiveTopologyType(
            D3D12_PRIMITIVE_TOPOLOGY_TYPE_TRIANGLE);
        m_GfxColorPSO[0][i].SetVertexShader(
            QuadVS->GetBufferPointer(), QuadVS->GetBufferSize());
        m_GfxColorPSO[0][i].SetRenderTargetFormats(
            1, &m_OutTexture[kColorTex].GetFormat(), DXGI_FORMAT_UNKNOWN);
        m_GfxColorPSO[0][i].SetPixelShader(
            CopyColorPS[0][i]->GetBufferPointer(), 
            CopyColorPS[0][i]->GetBufferSize());
        m_CptColorPSO[0][i].SetRootSignature(m_RootSignature);
        m_CptColorPSO[0][i].SetComputeShader(
...
void SensorTexGen::OnRender(CommandContext& EngineContext)
{
    if (m_Streaming) {
        ...
        for (int i = 0; i < IRGBDStreamer::kNumBufferTypes; ++i) {
            if ((i == IRGBDStreamer::kColor && m_ColorMode == kNoColor) &&
                (i == IRGBDStreamer::kDepth && m_DepthMode == kNoDepth) &&
                (i == IRGBDStreamer::kInfrared && m_DepthMode != 
                    kDepthWithVisualWithInfrared)) {
                continue;
            }
            m_pFrameAlloc[i]->DiscardBuffer(
                Graphics::g_stats.lastFrameEndFence, m_pKinectBuffer[i]);
            m_pKinectBuffer[i] = m_pFrameAlloc[i]->RequestFrameBuffer();
            ptrs[i] = 
                reinterpret_cast<uint8_t*>(m_pKinectBuffer[i]->GetMappedPtr());
        }

        // Acquire frame data from Kinect
        FrameData Frames[IRGBDStreamer::kNumBufferTypes];
        m_pKinect2->GetFrames(Frames[IRGBDStreamer::kColor], 
        ...
        if (m_UseCS) {
            ComputeContext& Context = EngineContext.GetComputeContext();
            {
                ...
                Context.SetDynamicConstantBufferView(
                    0, sizeof(RenderCB), &m_KinectCB);

                Context.SetDynamicDescriptors(1, 0, 1, 
                    &m_pKinectBuffer[IRGBDStreamer::kDepth]->GetSRV());
                Context.SetDynamicDescriptors(1, 1, 1, 
                    &m_pKinectBuffer[IRGBDStreamer::kInfrared]->GetSRV());
                Context.SetDynamicDescriptors(1, 2, 1, 
                    &m_pKinectBuffer[IRGBDStreamer::kColor]->GetSRV());

                Context.SetDynamicDescriptors(2, 0, 1, 
                    &m_OutTexture[kDepthTex].GetUAV());
                Context.SetDynamicDescriptors(2, 1, 1, 
                    &m_OutTexture[kDepthVisualTex].GetUAV());
                Context.SetDynamicDescriptors(2, 2, 1, 
                    &m_OutTexture[kInfraredTex].GetUAV());
                Context.SetDynamicDescriptors(2, 3, 1, 
                    &m_OutTexture[kColorTex].GetUAV());
                Context.SetPipelineState(
                    m_CptDepthPSO[m_DepthMode][m_ProcessMode]);
                Context.Dispatch1D(m_OutTexture[kDepthTex].GetWidth() * 
                    m_OutTexture[kDepthTex].GetHeight(), THREAD_PER_GROUP);

                if (m_ColorMode != kNoColor) {
                    Context.SetPipelineState(
                        m_CptColorPSO[m_ColorMode][m_ProcessMode]);
                    Context.Dispatch1D(m_OutTexture[kColorTex].GetWidth() * 
                        m_OutTexture[kColorTex].GetHeight(), THREAD_PER_GROUP);
                }
                Context.TransitionResource(m_OutTexture[kDepthVisualTex], 
                    D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
                Context.TransitionResource(m_OutTexture[kDepthTex], 
                    D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
                Context.TransitionResource(m_OutTexture[kInfraredTex], 
                    D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
                Context.TransitionResource(m_OutTexture[kColorTex], 
                    D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
            }
        } else {
            GraphicsContext& Context = EngineContext.GetGraphicsContext();
            {
                ...
                Context.SetRenderTargets(m_DepthMode + 1, m_OutTextureRTV);
                Context.SetViewports(1, &m_DepthInfraredViewport);
                Context.SetScisors(1, &m_DepthInfraredScissorRect);
                Context.SetPipelineState(
                    m_GfxDepthPSO[m_DepthMode][m_ProcessMode]);
                Context.Draw(3);

                Context.SetRenderTargets(1, &m_OutTextureRTV[kColorTex]);
...
void SensorTexGen::RenderGui()
{
    ...
    if (ImGui::CollapsingHeader("Sensor Tex Generator")) {
        ...
        for (int i = 1; i < kNumTargetTex; ++i) {
            if (ImGui::CollapsingHeader(TargetTexName[i].c_str())) {
                ...
                ImGui::SameLine();
                ImGui::Checkbox(("Show " + TargetTexName[i]).c_str(), 
                    &showImage[i]);
                ImGui::Image(tex_id, 
    ...
    if (m_DepthMode == kNoDepth && m_ColorMode == kNoColor) {
        m_DepthMode = kDepth;
    }
}
