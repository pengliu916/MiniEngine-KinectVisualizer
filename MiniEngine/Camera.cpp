...
OrbitCamera::OrbitCamera()
{
    ...
    V(SetPropertyInteractionContext(mInteractionContext,
        INTERACTION_CONTEXT_PROPERTY_FILTER_POINTERS, TRUE));
    {
        ...
        V(SetInteractionConfigurationInteractionContext(
            mInteractionContext, ARRAYSIZE(config), config));
    }
    V(SetInertiaParameterInteractionContext(mInteractionContext,
        INERTIA_PARAMETER_TRANSLATION_DECELERATION, 0.001f));
    V(RegisterOutputCallbackInteractionContext(mInteractionContext,
        OrbitCamera::_StaticInteractionOutputCallback, this));
}
...
void
OrbitCamera::View(DirectX::XMVECTOR center, float radius,
    float minRadius, float maxRadius, float longAngle, float latAngle)
{
    mCenter = center;
    mRadius = radius;
...
void
OrbitCamera::Projection(float fov, float aspect)
{
...
void
OrbitCamera::OrbitX(float angle)
{
    mLongAngle += angle;
    _UpdateData();
}
...
void
OrbitCamera::OrbitY(float angle)
{
...
void
OrbitCamera::ZoomRadius(float delta)
{
...
void
OrbitCamera::ZoomRadiusScale(float delta)
{
    mRadius = max(mMinRadius, min(mMaxRadius, mRadius * delta));
    _UpdateData();
}
...
void
OrbitCamera::AddPointer(uint32_t pointerId)
{
    AddPointerInteractionContext(mInteractionContext, pointerId);
}

void
OrbitCamera::ProcessPointerFrames(
    uint32_t pointerId, const POINTER_INFO* pointerInfo)
{
    ProcessPointerFramesInteractionContext(
        mInteractionContext, 1, 1, pointerInfo);
}

void
OrbitCamera::RemovePointer(uint32_t pointerId)
{
    RemovePointerInteractionContext(mInteractionContext, pointerId);
}

void
OrbitCamera::ProcessInertia()
{
    ProcessInertiaInteractionContext(mInteractionContext);
}

VOID CALLBACK
OrbitCamera::_StaticInteractionOutputCallback(
    VOID *clientData, const INTERACTION_CONTEXT_OUTPUT *output)
{
    auto camera = reinterpret_cast<OrbitCamera*>(clientData);
    camera->_InteractionOutputCallback(output);
}

void
OrbitCamera::_InteractionOutputCallback(
