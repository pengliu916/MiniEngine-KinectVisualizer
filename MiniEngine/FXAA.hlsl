...
//------------------------------------------------------------------------------
float
RGBToLogLuminance(float3 LinearRGB)
{
...
[numthreads(8, 8, 1)]
void
csmain(uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex,
    uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (GI < ROW_WIDTH * ROW_WIDTH / 2) {
        uint LdsCoord = GI;
        int2 UavCoord =
            uint2(GI % ROW_WIDTH, GI / ROW_WIDTH) + Gid.xy * 8 - BOUNDARY_SIZE;
    ...
    uint CenterIdx =
        (GTid.x + BOUNDARY_SIZE) + (GTid.y + BOUNDARY_SIZE) * ROW_WIDTH;
    ...
    float edgeHorz = abs(lumaNWSW - 2.0 * lumaW) +
        abs(lumaNS - 2.0 * lumaM) * 2.0 + abs(lumaNESE - 2.0 * lumaE);
    float edgeVert = abs(lumaSWSE - 2.0 * lumaS) +
        abs(lumaWE - 2.0 * lumaM) * 2.0 + abs(lumaNWNE - 2.0 * lumaN);

    // Also compute local contrast in the 3x3 region.
    // This can identify standalone pixels that alias.
    float avgNeighborLuma =
        ((lumaNS + lumaWE) * 2.0 + lumaNWSW + lumaNESE) / 12.0;
    float subpixelShift = saturate(pow(smoothstep(
        0, 1, abs(avgNeighborLuma - lumaM) / range), 2) * SubpixelRemoval * 2);
...
// Note that the number of samples in each direction is one less than the
// number of sample distances.  The last is the maximum distance that should
// be used, but whether that sample is "good" or "bad" doesn't affect the
// result, so we don't need to load it.
    ...
    uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID)
{
    ...
    // Determines which side of the pixel has the highest contrast
    uint GradientDir = WorkHeader & 1;
    // 7-bits to encode [0, 0.5]
    float Subpix = (WorkHeader & 0xFE) / 254.0 * 0.5;
    ...
    // Halfway between this and next; center of the contrasting edge
    float CenterLuma = (NextLuma + ThisLuma) * 0.5;
    // Going down in brightness or up?
    float GradientSgn = sign(NextLuma - ThisLuma);
    // How much contrast?  When can we stop looking?
    float GradientMag = abs(NextLuma - ThisLuma) * 0.25;
