...
template <typename T> __forceinline T
DivideByMultiple(T value, size_t alignment)
{
...
if(!(bool)(isTrue)){ \
    PRINTERROR("Assertion failed in" STRINGIFY_BUILTIN(__FILE__) \
    " @ " STRINGIFY_BUILTIN(__LINE__)"\n \t \'"#isTrue"\' is false."); \
    __debugbreak(); \
}
#endif
...
inline void
DX_SetDebugName(_In_ IDXGIObject* pObj, _In_z_ const WCHAR* pwcsName)
{
    if (pObj) {
        pObj->SetPrivateData(WKPDID_D3DDebugObjectNameW,
            (uint32_t)wcslen(pwcsName) * 2, pwcsName);
    }
}

inline void
DX_SetDebugName(_In_ ID3D12Device* pObj, _In_z_ const WCHAR* pwcsName)
{
    if (pObj) {
...
inline void
DX_SetDebugName(_In_ ID3D12Resource* pObj, _In_z_ const WCHAR* pwcsName)
{
    if (pObj) {
        pObj->SetName(pwcsName);
    }
...
inline void
DX_SetDebugName(_In_ ID3D12DeviceChild* pObj, _In_z_ const WCHAR* pwcsName)
{
    if (pObj) {
        pObj->SetName(pwcsName);
    }
...
inline size_t
HashIterate(
    size_t Next, size_t CurrentHash = 2166136261U /*Offset for 32bit version*/)
{
    return 16777619U/*32bit FNV_Prim*/ * CurrentHash ^ Next;
}

template<typename T> inline size_t
HashRange(const T* Begin, const T* End, size_t InitialVal = 2166136261U)
{
...
template<typename T> inline size_t
HashStateArray(
    const T* StateDesc, size_t Count, size_t InitialVal = 2166136261U)
{
...
template<typename T> inline size_t
HashState(const T* StateDesc, size_t InitialVal = 2166136261U)
{
    static_assert((sizeof(T) & 3) == 0, "State object is not word-aligned");
