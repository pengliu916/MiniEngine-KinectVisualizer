...
struct CD3D12_SHADER_BYTECODE : public D3D12_SHADER_BYTECODE
{
    ...
    explicit CD3D12_SHADER_BYTECODE(
        const void* _pShaderBytecode, SIZE_T _BytecodeLength) {
...
public:
    ...
    void SetRenderTargetFormats(UINT NumRTVs, const DXGI_FORMAT* RTVFormats,
        DXGI_FORMAT DSVFormat, UINT MsaaCount = 1, UINT MsaaQuality = 0);
    void SetInputLayout(UINT NumElements,
        const D3D12_INPUT_ELEMENT_DESC* pInputElementDescs);
    void SetPrimitiveRestart(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE IBProps);

    void SetVertexShader(const void* Binary, size_t Size) {
        m_PSODesc.VS = CD3D12_SHADER_BYTECODE(Binary, Size);
    }
    void SetPixelShader(const void* Binary, size_t Size) {
        m_PSODesc.PS = CD3D12_SHADER_BYTECODE(Binary, Size);
    }
    void SetGeometryShader(const void* Binary, size_t Size) {
        m_PSODesc.GS = CD3D12_SHADER_BYTECODE(Binary, Size);
    }
    void SetHullShader(const void* Binary, size_t Size) {
        m_PSODesc.HS = CD3D12_SHADER_BYTECODE(Binary, Size);
    }
    void SetDomainShader(const void* Binary, size_t Size) {
        m_PSODesc.DS = CD3D12_SHADER_BYTECODE(Binary, Size);
    }
    ...
    // Perform validation and compute a hash value for fast
    // state block comparisons
    void Finalize();
...
public:
    ...
    void SetComputeShader(const void* Binary, size_t Size) {
        m_PSODesc.CS = CD3D12_SHADER_BYTECODE(Binary, Size);
    }
