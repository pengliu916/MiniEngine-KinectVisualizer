...
{
    return DescribeTex(Width, Height, DepthOrArraySize, NumMips,
        Format, Flags, D3D12_RESOURCE_DIMENSION_TEXTURE2D);
}

D3D12_RESOURCE_DESC
PixelBuffer::DescribeTex3D(uint32_t Width, uint32_t Height, uint32_t Depth,
    uint32_t NumMips, DXGI_FORMAT Format, UINT Flags)
{
    return DescribeTex(Width, Height, Depth, NumMips,
        Format, Flags, D3D12_RESOURCE_DIMENSION_TEXTURE3D);
}

D3D12_RESOURCE_DESC
PixelBuffer::DescribeTex(uint32_t Width, uint32_t Height,
    uint32_t DepthOrArraySize, uint32_t NumMips, DXGI_FORMAT Format,
    UINT Flags, D3D12_RESOURCE_DIMENSION Dimension)
{
    m_Width = Width;
    m_Height = Height;
...
void
PixelBuffer::AssociateWithResource(
...
void
PixelBuffer::CreateTextureResource(
    ID3D12Device* Device, const std::wstring& Name,
...
DXGI_FORMAT
PixelBuffer::GetBaseFormat(DXGI_FORMAT defaultFormat)
{
...
DXGI_FORMAT
PixelBuffer::GetUAVFormat(DXGI_FORMAT defaultFormat)
{
    switch (defaultFormat) {
...
DXGI_FORMAT
PixelBuffer::GetDSVFormat(DXGI_FORMAT defaultFormat)
{
    switch (defaultFormat) {
        // 32-bit Z w/ Stencil
    case DXGI_FORMAT_R32G8X24_TYPELESS:
...
DXGI_FORMAT
PixelBuffer::GetDepthFormat(DXGI_FORMAT defaultFormat)
{
    switch (defaultFormat) {
        // 32-bit Z w/ Stencil
    case DXGI_FORMAT_R32G8X24_TYPELESS:
...
DXGI_FORMAT
PixelBuffer::GetStencilFormat(DXGI_FORMAT defaultFormat)
{
    switch (defaultFormat) {
        // 32-bit Z w/ Stencil
    case DXGI_FORMAT_R32G8X24_TYPELESS:
...
ColorBuffer::CreateFromSwapChain(
    const std::wstring& Name, ID3D12Resource* BaseResource)
{
    AssociateWithResource(Graphics::g_device.Get(), Name,
        BaseResource, D3D12_RESOURCE_STATE_PRESENT);
    if (m_RTVHandle.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) {
        m_RTVHandle = Graphics::g_pRTVDescriptorHeap->Append().GetCPUHandle();
    }
    Graphics::g_device->CreateRenderTargetView(
        m_pResource.Get(), nullptr, m_RTVHandle);
}

void
ColorBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height,
    ...
    D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr /*= D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN*/)
{
    ...
    CreateTextureResource(
        Graphics::g_device.Get(), Name, ResourceDesc, ClearValue, VidMemPtr);
...
void
ColorBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format,
    uint32_t ArraySize, uint32_t NumMips /* = 1 */)
{
    ...
    for (uint32_t i = 0; i < NumMips; ++i) {
        ...
        Device->CreateUnorderedAccessView(
            Resource, nullptr, &UAVDesc, m_UAVHandle[i]);
...
//------------------------------------------------------------------------------
void
DepthBuffer::Create(const std::wstring& Name,
    uint32_t Width, uint32_t Height, DXGI_FORMAT Format,
    D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr /*= D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN*/)
{
    D3D12_RESOURCE_DESC ResourceDesc = DescribeTex2D(Width, Height, 1, 1,
        Format, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
    D3D12_CLEAR_VALUE ClearValue = {};
    ClearValue.Format = Format;
    ClearValue.DepthStencil.Depth = m_ClearDepth;
    ClearValue.DepthStencil.Stencil = m_ClearStencil;
    CreateTextureResource(Graphics::g_device.Get(), Name,
        ResourceDesc, ClearValue, VidMemPtr);
    CreateDerivedViews(Graphics::g_device.Get(), Format);
}

void
DepthBuffer::Create(const std::wstring& Name, uint32_t Width, uint32_t Height,
    uint32_t Samples, DXGI_FORMAT Format, D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr)
{
    D3D12_RESOURCE_DESC ResourceDesc = DescribeTex2D(Width, Height, 1, 1,
        Format, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
...
void
DepthBuffer::CreateDerivedViews(ID3D12Device* Device, DXGI_FORMAT Format)
{
    ID3D12Resource* Resource = m_pResource.Get();
    ...
    if (m_DSVHandle[0].ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) {
        m_DSVHandle[0] =
            Graphics::g_pDSVDescriptorHeap->Append().GetCPUHandle();
        m_DSVHandle[1] =
            Graphics::g_pDSVDescriptorHeap->Append().GetCPUHandle();
    }
    ...
    if (stencilReadFormat != DXGI_FORMAT_UNKNOWN) {
        ...
        Device->CreateShaderResourceView(
            Resource, &SRVDesc, m_StencilSRVHandle);
    }
}
...
//------------------------------------------------------------------------------
void
VolumeTexture::Create(const std::wstring& Name, uint32_t Width, uint32_t Height,
    uint32_t Depth, uint32_t NumMips, DXGI_FORMAT Format,
    D3D12_GPU_VIRTUAL_ADDRESS VidMemPtr /*= D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN*/)
{
    NumMips = (NumMips == 0 ? ComputeNumMips(Width, Height) : NumMips);
    D3D12_RESOURCE_DESC Desc = DescribeTex3D(Width, Height, Depth, NumMips,
...
void
VolumeTexture::CreateDerivedViews(DXGI_FORMAT Format)
{
    D3D12_RENDER_TARGET_VIEW_DESC RTVDesc = {};
    D3D12_UNORDERED_ACCESS_VIEW_DESC UAVDesc = {};
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    ...
    Graphics::g_device->CreateShaderResourceView(
        Resource, &SRVDesc, m_SRVHandle);
    if (m_RTVHandle.ptr == D3D12_GPU_VIRTUAL_ADDRESS_UNKNOWN) {
        m_RTVHandle = Graphics::g_pRTVDescriptorHeap->Append().GetCPUHandle();
    }
    Graphics::g_device->CreateRenderTargetView(
        Resource, &RTVDesc, m_RTVHandle);
    ...
    Graphics::g_device->CreateUnorderedAccessView(
        Resource, nullptr, &UAVDesc, m_UAVHandle);
}
...
void
GpuBuffer::Create(const std::wstring& Name, uint32_t NumElements,
...
void
GpuBuffer::CreatePlaced(const std::wstring& Name, ID3D12Heap* BackingHeap,
    uint32_t HeapOffset, uint32_t NumElements,
    uint32_t ElementSize, const void* InitData /* = nullptr */)
{
    m_ElementCount = NumElements;
    m_ElementSize = ElementSize;
    ...
    D3D12_RESOURCE_DESC ResourceDesc = DescribeBuffer();
    m_UsageState = D3D12_RESOURCE_STATE_COMMON;
    V(Graphics::g_device->CreatePlacedResource(
        BackingHeap, HeapOffset, &ResourceDesc, m_UsageState,
...
GpuBuffer::CreateConstantBufferView(uint32_t Offset, uint32_t Size) const
{
...
//------------------------------------------------------------------------------
void
ByteAddressBuffer::CreateDerivedViews()
{
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    ...
    Graphics::g_device->CreateUnorderedAccessView(
        m_pResource.Get(), nullptr, &UAVDesc, m_UAV);
}
...
void
StructuredBuffer::CreateDerivedViews()
{
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    SRVDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
    ...
    Graphics::g_device->CreateUnorderedAccessView(
        m_pResource.Get(), m_CounterBuffer.GetResource(), &UAVDesc, m_UAV);
}

ByteAddressBuffer&
StructuredBuffer::GetCounterBuffer()
{
    return m_CounterBuffer;
}

const D3D12_CPU_DESCRIPTOR_HANDLE&
StructuredBuffer::GetCounterSRV(CommandContext& Context)
{
    Context.TransitionResource(
        m_CounterBuffer, D3D12_RESOURCE_STATE_GENERIC_READ);
    return m_CounterBuffer.GetSRV();
}

const D3D12_CPU_DESCRIPTOR_HANDLE&
StructuredBuffer::GetCounterUAV(CommandContext& Context)
{
    Context.TransitionResource(
        m_CounterBuffer, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
...
//------------------------------------------------------------------------------
void
TypedBuffer::CreateDerivedViews()
{
    D3D12_SHADER_RESOURCE_VIEW_DESC SRVDesc = {};
    SRVDesc.ViewDimension = D3D12_SRV_DIMENSION_BUFFER;
    ...
    DXGI_FORMAT Format, const void* InitData)
{
    ...
    HRESULT hr;
    V(Graphics::g_device->CreateCommittedResource(
        &HeapProps, D3D12_HEAP_FLAG_NONE, &textDesc, m_UsageState, nullptr,
