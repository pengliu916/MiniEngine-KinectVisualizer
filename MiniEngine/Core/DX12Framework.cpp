...
namespace
{
    ...
    // In multi-thread scenario, current thread may read old version of the
    // following boolean due to unflushed cache etc. So to use flag in
    // multi-thread cases, atomic bool is needed, and memory order
    // semantic is crucial 
    ...
    LRESULT CALLBACK WindowProc(
        ...
        case WM_SIZE:
            if (wParam != SIZE_MINIMIZED) {
                ...
                if (g_config.swapChainDesc.Width != _width ||
        ...
        while (!_terminated && !_hasError) {
            ...
            // Update GUI
            {
                ...
                swprintf(buffer, 512, L"-%4.1f ms  %.0f fps %s",
                    1000.f * frameTime, 1.0f / frameTime, g_strCustom);
