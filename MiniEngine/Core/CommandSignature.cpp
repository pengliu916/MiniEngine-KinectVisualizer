...
void
IndirectParameter::IndexBufferView()
{
    m_IndirectParam.Type = D3D12_INDIRECT_ARGUMENT_TYPE_INDEX_BUFFER_VIEW;
}

void
IndirectParameter::Constant(UINT RootParameterIndex,
...
void
IndirectParameter::ConstantBufferView(UINT RootParameterIndex)
{
...
void
IndirectParameter::ShaderResourceView(UINT RootParameterIndex)
{
...
void
IndirectParameter::UnorderedAccessView(UINT RootParameterIndex)
{
...
void
CommandSignature::Reset(UINT NumParams)
{
    if (NumParams > 0) {
...
IndirectParameter&
CommandSignature::operator[] (size_t EntryIndex)
{
    ASSERT(EntryIndex < m_NumParameters);
    return m_ParamArray.get()[EntryIndex];
}

const IndirectParameter&
CommandSignature::operator[](size_t EntryIndex) const
{
    ASSERT(EntryIndex < m_NumParameters);
    return m_ParamArray.get()[EntryIndex];
}
...
void
CommandSignature::Finalize(const RootSignature* RootSignature /* = nullptr */)
{
    if (m_Finalized) {
        return;
    }
    ...
    CommandSignatureDesc.pArgumentDescs =
        (const D3D12_INDIRECT_ARGUMENT_DESC *)m_ParamArray.get();
    CommandSignatureDesc.NodeMask = 1;

    Microsoft::WRL::ComPtr<ID3DBlob> pOutBlob, pErrorBlob;
    ID3D12RootSignature* pRootSig =
        RootSignature ? RootSignature->GetSignature() : nullptr;
    ...
    HRESULT hr;
    V(Graphics::g_device->CreateCommandSignature(
        &CommandSignatureDesc, pRootSig, IID_PPV_ARGS(&m_Signature)));
    m_Signature->SetName(L"CommandSignature");
    m_Finalized = TRUE;
}

ID3D12CommandSignature*
CommandSignature::GetSignature() const
{
