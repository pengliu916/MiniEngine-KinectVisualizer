...
void
CommandAllocatorPool::Create(ID3D12Device* pDevice)
{
    m_pDevice = pDevice;
}
...
CommandAllocatorPool::RequestAllocator(uint64_t CompletedFenceValue)
{
    ...
    if (!m_ReadyAllocators.empty()) {
        std::pair<uint64_t, ID3D12CommandAllocator*>& AllocatorPair =
            m_ReadyAllocators.front();
        if (AllocatorPair.first <= CompletedFenceValue) {
            pAllocator = AllocatorPair.second;
            V(pAllocator->Reset());
            m_ReadyAllocators.pop();
        }
        Graphics::g_stats.allocatorReady[m_cCommandListType] =
            (uint16_t)m_ReadyAllocators.size();
    }
    if (pAllocator == nullptr) {
        V(m_pDevice->CreateCommandAllocator(
            m_cCommandListType, IID_PPV_ARGS(&pAllocator)));
        wchar_t AllocatorName[32];
        swprintf(AllocatorName, 32, L"CommandAllocator %zu",
            m_AllocatorPool.size());
        pAllocator->SetName(AllocatorName);
        m_AllocatorPool.push_back(pAllocator);
        Graphics::g_stats.allocatorCreated[m_cCommandListType] =
            (uint16_t)m_AllocatorPool.size();
    }
    return pAllocator;
}

void
CommandAllocatorPool::DiscardAllocator(
    uint64_t FenceValue, ID3D12CommandAllocator* Allocator)
{
    CriticalSectionScope LockGuard(&m_AllocatorCS);
...
void
CommandQueue::Create(ID3D12Device* pDevice)
{
...
bool
CommandQueue::IsFenceCompelete(uint64_t FenceValue)
{
    if (FenceValue > m_LastCompletedFenceValue) {
        m_LastCompletedFenceValue =
            max(m_LastCompletedFenceValue, m_pFence->GetCompletedValue());
    }
    return FenceValue <= m_LastCompletedFenceValue;
}

void
CommandQueue::WaitForFence(uint64_t FenceValue)
{
    ...
    }
    {
        ...
        Graphics::g_stats.cpuStallTimePerFrame +=
            (double)(endTick - startTick) / Core::g_tickesPerSecond * 1000.f;
...
uint64_t
CommandQueue::_ExecuteCommandList(ID3D12CommandList* List)
{
    CriticalSectionScope LockGuard(&m_FenceCS);
    HRESULT hr;
...
ID3D12CommandAllocator*
CommandQueue::_RequestAllocator()
{
...
void
CommandQueue::_DiscardAllocator(
    uint64_t FenceValueForReset, ID3D12CommandAllocator* Allocator)
{
...
void
CmdListMngr::Create(ID3D12Device* pDevice)
{
    ASSERT(pDevice != nullptr);
    m_pDevice = pDevice;
#ifndef RELEASE
    // Prevent the GPU from overclocking or underclocking to get
    // consistent timings
...
CommandQueue&
CmdListMngr::GetGraphicsQueue()
{
    return m_GraphicsQueue;
}

CommandQueue&
CmdListMngr::GetComputeQueue()
{
    return m_ComputeQueue;
}

CommandQueue&
CmdListMngr::GetCopyQueue()
{
    return m_CopyQueue;
}

CommandQueue&
CmdListMngr::GetQueue(
...
ID3D12CommandQueue*
CmdListMngr::GetCommandQueue()
{
    return m_GraphicsQueue.GetCommandQueue();
}

void
CmdListMngr::CreateNewCommandList(D3D12_COMMAND_LIST_TYPE Type,
    ID3D12GraphicsCommandList** List, ID3D12CommandAllocator** Allocator)
{
    ...
    case D3D12_COMMAND_LIST_TYPE_DIRECT:
        *Allocator = m_GraphicsQueue._RequestAllocator();
        break;
    case D3D12_COMMAND_LIST_TYPE_BUNDLE:
        break;
    case D3D12_COMMAND_LIST_TYPE_COMPUTE:
        *Allocator = m_ComputeQueue._RequestAllocator();
        break;
    case D3D12_COMMAND_LIST_TYPE_COPY:
        *Allocator = m_CopyQueue._RequestAllocator();
        break;
    }
    HRESULT hr;
    V(m_pDevice->CreateCommandList(
        1, Type, *Allocator, nullptr, IID_PPV_ARGS(List)));
    (*List)->SetName(L"CommandList");
}

bool
CmdListMngr::IsFenceComplete(uint64_t FenceValue)
{
    return GetQueue(D3D12_COMMAND_LIST_TYPE(FenceValue >> 56))
        .IsFenceCompelete(FenceValue);
}

void
CmdListMngr::WaitForFence(uint64_t FenceValue)
{
    GetQueue((D3D12_COMMAND_LIST_TYPE)(FenceValue >> 56))
