...
static unordered_map<size_t, ComPtr<ID3D12PipelineState>> s_GraphicsPSOHashMap;
static unordered_map<size_t, ComPtr<ID3D12PipelineState>> s_ComputePSOHashMap;

static CRITICAL_SECTION s_GraphicsPSOCS;
static CRITICAL_SECTION s_ComputePSOCS;
...
void
PSO::SetRootSignature(const RootSignature& BindMappings)
{
    m_RootSignature = &BindMappings;
}

const RootSignature&
PSO::GetRootSignature() const
{
...
ID3D12PipelineState*
PSO::GetPipelineStateObject() const
{
    return m_PSO;
}
...
void
GraphicsPSO::SetBlendState(const D3D12_BLEND_DESC& BlendDesc)
{
    m_PSODesc.BlendState = BlendDesc;
}

void
GraphicsPSO::SetRasterizerState(const D3D12_RASTERIZER_DESC& RasterizerDesc)
{
    m_PSODesc.RasterizerState = RasterizerDesc;
}

void
GraphicsPSO::SetDepthStencilState(
...
void
GraphicsPSO::SetSampleMask(UINT SampleMask)
{
    m_PSODesc.SampleMask = SampleMask;
}

void
GraphicsPSO::SetPrimitiveTopologyType(
...
void
GraphicsPSO::SetRenderTargetFormats(UINT NumRTVs, const DXGI_FORMAT* RTVFormats,
...
void
GraphicsPSO::SetInputLayout(
    UINT NumElements, const D3D12_INPUT_ELEMENT_DESC* pInputElementDescs)
{
    m_PSODesc.InputLayout.NumElements = NumElements;
    if (NumElements > 0) {
        D3D12_INPUT_ELEMENT_DESC* NewElements =
            (D3D12_INPUT_ELEMENT_DESC*)malloc(
                sizeof(D3D12_INPUT_ELEMENT_DESC)*NumElements);
        memcpy(NewElements, pInputElementDescs,
            NumElements * sizeof(D3D12_INPUT_ELEMENT_DESC));
...
void
GraphicsPSO::SetPrimitiveRestart(D3D12_INDEX_BUFFER_STRIP_CUT_VALUE IBProps)
{
...
GraphicsPSO::Finalize()
{
    ...
    HashCode = HashStateArray(
        m_InputLayouts.get(), m_PSODesc.InputLayout.NumElements, HashCode);
    ...
    if (firstCompile) {
        HRESULT hr;
        V(Graphics::g_device->CreateGraphicsPipelineState(
...
ComputePSO::Finalize()
{
    ...
    if (firstCompile) {
        HRESULT hr;
        V(Graphics::g_device->CreateComputePipelineState(
            &m_PSODesc, IID_PPV_ARGS(&m_PSO)));
