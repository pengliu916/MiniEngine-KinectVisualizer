...
namespace
{
    ...
    // engine:- in your Render function, try translating your projection 
    ...
    void ImGuiRender(ImDrawData* draw_data, GraphicsContext& context) {
        ...
        context.SetRenderTargets(1,
            &Graphics::g_pDisplayPlanes[Graphics::g_CurrentDPIdx].GetRTV());
    ...
    void CreateFontsTexture() {
        ...
        _texture.Create(L"ImGui Font Texture",
...
GuiRenderer::OnEvent(MSG* msg)
{
    ...
    case WM_MOUSEWHEEL:
        io.MouseWheel +=
            GET_WHEEL_DELTA_WPARAM(msg->wParam) > 0 ? +1.0f : -1.0f;
        break;
    case WM_MOUSEMOVE:
...
GuiRenderer::CreateResource()
{
    ...
    _rootSignature.InitStaticSampler(
        0, Graphics::g_SamplerLinearWrapDesc, D3D12_SHADER_VISIBILITY_PIXEL);
    ...
    VRET(D3DCompile(vertexShader, strlen(vertexShader), NULL, NULL, NULL,
        "main", "vs_5_1", 0, 0, &vertexShaderBlob, NULL));
    ...
    VRET(D3DCompile(pixelShader, strlen(pixelShader), NULL, NULL, NULL,
        "main", "ps_5_1", 0, 0, &pixelShaderBlob, NULL));
    ...
    _graphicsPSO.SetRenderTargetFormats(
...
GuiRenderer::NewFrame()
{
    ...
    io.DisplaySize = ImVec2(
        (float)(Core::g_config.swapChainDesc.Width),
        (float)(Core::g_config.swapChainDesc.Height));
