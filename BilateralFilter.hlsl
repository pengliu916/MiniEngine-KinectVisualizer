...
#define GAUSSIAN_VARIANCE ((KERNEL_RADIUS+1.f)*(KERNEL_RADIUS+1.f)*0.25f) // Cutoff at 2*deviation

Texture2D<uint>	inputTex  : register(t0);

#if QuadVS
void vs_quad_main( in uint VertID : SV_VertexID, out float2 Tex : TEXCOORD0, out float4 Pos : SV_Position )
{
